...
def _req(fn):
    def wrapper(device, path, **kwargs):
        ...
        url = f"{BASE_URL}/v1/devices/{device.chip_id}{path}"
...
def start_calibration(device, *args, **kwargs):
    return _call_func(device, "initCalibration")

@_particle_func
def calibrate(device, cal_weight, *args, **kwargs):
    return _call_func(device, "calibrate", cal_weight)

@_particle_func
def tare(device, *args, **kwargs):
    return _call_func(device, "tare")

@_particle_func
def clear_memory(device, *args, **kwargs):
    return _call_func(device, "clearMemory")

@_particle_func
def send_most_recent_sample(device,  *args, **kwargs):
    return _call_func(device, "sendMostRecentSample")

@_particle_func
def start_maintenance_mode(device, *args, **kwargs):
    return _call_func(device, "startMaintenanceMode")

@_particle_func
def stop_maintenance_mode(device, *args, **kwargs):
    return _call_func(device, "stopMaintenanceMode")
