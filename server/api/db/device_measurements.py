...
from db import Base, DictifiableMixin, QueryMethodsMixin, devices
...
class DeviceMeasurements(Base, DictifiableMixin, QueryMethodsMixin):
    ...
    device_id = Column(UUID, ForeignKey(f"{devices._TABLE_NAME}.{devices._PKEY}"), nullable=False)
    measurement = Column(Float, nullable=False)
    unit = Column(String, nullable=False)
    taken_on = Column(DateTime(timezone=True), nullable=False)

    device = relationship(devices.Devices, backref=backref("DeviceMeasurements", cascade="all,delete"))
    ...
    @classmethod
    def get_latest_measurement(cls, session, device_id, measure, **kwargs):
        return session.query(cls).filter_by(device_id=device_id, measure=measure, **kwargs)